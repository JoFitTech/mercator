...
# Idempotente Nachruest-Spalten fuer Bestandsdatenbanken.
# Diese Liste deckt den aktuellen insider_trades-Sollzustand ab (ohne AUTO_INCREMENT id).
INSIDER_TRADES_MIGRATION_COLUMNS: list[tuple[str, str]] = [
    ...
    def initialize_schema(self) -> list[str]:
        ...
        with self.connection(include_database=True) as conn:
            with conn.cursor() as cursor:
                ...
                # --- insider_trades ---
                for col_name, col_def in INSIDER_TRADES_MIGRATION_COLUMNS:
