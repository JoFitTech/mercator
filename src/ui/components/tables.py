...
def render_trade_table(df: pd.DataFrame, height: int = 600, on_select: str = "rerun") -> Any:
    ...
    visible_cols = ["symbol_at_trade", "reporting_name", "direction", "trade_value_estimated", "score_value", "transaction_date"]
    ...
    col_config = {
        ...
        "trade_value_estimated": st.column_config.NumberColumn("Value", format="$%d", width="small"), # %d für weniger Platz
        "score_value": st.column_config.NumberColumn("Score", format="%.1f", width="small"), # %.1f spart Platz
        ...
        "transaction_date": st.column_config.DateColumn("Date", width="small", format="DD.MM.YY"), # Kompakteres Datum
    }
