...
def render_companies_page(
    repository: CompanyMySqlRepository | None,
    ...
    feedback_kind = str(st.session_state.pop("companies_feedback_kind", "success") or "success")
    if feedback:
    ...
    unresolved_count = int(
        df.get("profile_status", pd.Series(dtype="object")).apply(_normalize_profile_status).ne("FETCHED").sum()
    ) if "profile_status" in df.columns else 0
    ...
    work_df["profile_status"] = work_df["profile_status"].apply(_profile_status_label)
    ...
    work_df["profil_vollstaendig"] = source_df.apply(
        lambda row: "Ja" if not _is_incomplete_profile(row.to_dict()) else "Nein",
        axis=1,
    )
    display_df = work_df.drop(columns=["__row_id", "description"])
    ...
    event = st.dataframe(
        ...
        column_config={
            ...
            "profile_status": st.column_config.TextColumn("Profilstatus"),
            "profil_vollstaendig": st.column_config.TextColumn("Profil vollständig"),
    ...
    if selected_idx is not None:
        ...
        with st.container(border=True):
            ...
            st.caption(f"Profilstatus: {profile_status}")
            if missing_fields:
                st.caption(f"Fehlt: {', '.join(missing_fields)}")
            ...
            with c3:
                if st.button(
                    ...
                    help=(refresh_block_reason or "") if not can_refresh_button else None,
                ):
                    with st.spinner("Lade Unternehmensprofil über API2 nach..."):
                        result = _refresh_company_profile(import_service, symbol_value)  # type: ignore[arg-type]
                    if bool(result.get("ok")):
                        st.session_state["companies_feedback"] = f"Profil für {symbol_value} wurde aktualisiert."
                        st.session_state["companies_feedback_kind"] = "success"
                    else:
                        st.session_state["companies_feedback"] = str(
                            result.get("message") or "Profilaktualisierung fehlgeschlagen."
                        )
                        st.session_state["companies_feedback_kind"] = "error"
                    st.rerun()
